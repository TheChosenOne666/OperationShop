...
《玩法与技术方案》写了大量 v2 内容，**以下条目在 M01 代码中尚未实现**，按规划文档的模块归属归档。
...
| B5.3 校验规则调整 | B5.3 | **无需调整**（2026-09-14：关店移除后 `prepared` 单调递增、且「未解锁 ⇒ 未开业」，M01 现有「未准备店铺不得有累计客流」规则即最终形态；仅分段/金币恒等式随 ADR 0002 主决议在 M02 改写） | M02 |

### 7.2 代码与文档的**冲突/不一致**

> 这些不是"还没做"，而是**代码与文档互相矛盾**或**两份文档互相矛盾**，需要主理人拍板或排期修正。
> **状态图例（2026-09-14 复核）**：每条现标注其当前状态——**仍成立**（待 M02/后续处理）／**已决议**（方向已定、待实现）／**已解决**（差异已消失，仅留历史记录）。本表最易积"僵尸条目"，**变更发生后请回表核对**。

| 编号 | 冲突 | 细节 | 建议 |
| --- | --- | --- | --- |
| **A** | **客流轮询顺序不一致** | **代码**：轮询按配置顺序 `clothing → dessert → bookstore → coffee → flowers`（衣橱→甜屋→书店→咖啡→花束，见 `config/development.json` 与 `TestServiceRoundRobin`）。**草案 A4**：写明顺序为 `咖啡 → 花束 → 衣橱 → 甜屋 → 书店`。两者顺序不同。 | **方向已决议**（2026-09-14：**一层临街优先**，见草案 A4 与系统 GDD《经营与成长》§2.3）：M02 需把 `config/development.json` 的 `shops` 数组改为 `咖啡 → 花束 → 衣橱 → 甜屋 → 书店`；改后本冲突消解（配置变更会改规则指纹、拒绝旧档，属预期） |
...
| **C** | **未开业店铺校验规则** | ~~**代码** `validateState`：`(!shop.Prepared && shop.Visitors != 0)` → 拒绝"未准备店铺有累计客流"。**草案 B5.3**：放宽为"**未解锁铺位**不得有累计客流与收益"（因为关店要保留累计）。**当前若引入关店，会直接触发"关店即拒档"**。~~ **该差异已消解（2026-09-14）**：关店功能被移除后 `prepared` 单调递增（只可能 false→true），且「未解锁 ⇒ 未开业」（`prepare` 对未解锁铺位返回 `SLOT_LOCKED`），M01 现有校验 `(!shop.Prepared && shop.Visitors != 0)` **即最终形态，不放宽**。口径以草案 B5.3（2026-09-14 修正）与系统 GDD《经营与成长》§5.5 为准。 | **无需修正**：ADR 0002 的「随附决策（放宽）」已同步作废，代码与文档不再不一致 |
| **D** | **微信登录的模块归属互相矛盾** | ~~**草案**：`POST /api/v1/session/wechat` 与 B5.5 标注 **M06**。~~ **《开发规划文档》**：微信登录与云端存档为 **M07**。 | **已解决（2026-09-14）**：草案 v1.1 已把 B4.2 接口表与 B5.5 均改标 **M07**，与规划文档一致。本行保留作历史记录 |
| **E** | **写操作日志口径** | **草案 B5.6（2026-09-14 修正为三类）**：**结算、解锁、升级**三类写操作都要有可追踪日志（含结果码）。**代码**：只有通用访问日志（`method` / `path` / `duration_ms`），未按操作类型或结果码分类。 | M02 补结构化业务日志，或明确接受"以 path 追踪" |
| **F** | **店铺标识体系（shop vs slot）** | **代码**：以 shop ID（`clothing`/`dessert`/…）为核心，`floor`/`slot` 只是描述。**草案 B4.2**：解锁接口用 **slotId**（`f2-s1` 形式）。v2 需要引入独立的"铺位"维度并与现有 shop ID 建立映射。 | **设计侧已定**（系统 GDD《经营与成长》§3.2）：对外接口用 **slotId**（解锁/解锁校验）、存档与轮询用 **shopId**，二者经 `slots[].shopId` 一对一映射，**两套 ID 不得混用**；M02 实现时遵循即可 |
| **G** | **"剩余客流" vs "上限"暴露** | **代码**：`GET /api/v1/mall` 返回 `VisitorsRemaining`（剩余），**不直接返回上限**；客户端要显示 HUD 分母需自行结合 `/api/v1/config`。**HUD 语义已拍板**（2026-09-14：`36/50` = 「**已到店 / 上限**」，见需求文档 §6.1 与系统 GDD《经营与成长》§6.1；原"待确认项 2"已结案并移出）。 | **语义已定，剩余待定的是快照形态**：M02 是否在 `/api/v1/mall` 快照里**直接暴露** `dailyVisitorCap` 与已到店数（`visitorsServed`），而非让客户端自己结合 `/api/v1/config` 推算。系统 GDD §6.1 与附录 A 冲突⑧ 均**建议直接暴露** |
