...
| **K** | **B3.6「回前台拉快照」的括注与 `Snapshot()` 实际语义矛盾**（M05 实现发现，`SC-M05-QA-001 P2-c`） | 草案 B3.6 写「回到前台立即拉一次快照（**服务端会把这段时间的客流算出来**）」，但 `Service.Snapshot()`（`internal/game/service.go:255`）的注释明写「not settling income, not refreshing a day」——只读接口不产生增量。照字面实现，后台期间的客流要等下一轮 tick（≤5 秒）才算得出来，HUD 业务日也可能停在上一天 | **仍成立（GDD 侧待修）**。M05 取该句**意图**：`resume()` 发 `POST /mall/settle`（返回体自带最新快照，一次请求同时完成取状态与补算），启动 `load()` 仍按字面走 `GET /mall`；偏差登记在 `docs/M05-经营闭环.md` §9-⑧。**建议改 B3.6 那句括注**为「回前台立即结算一次」，别让实现长期背着"与字面不符"的债 |
| **L** | **未冻结时 HUD 客流分子显示口径两套**（M05 交叉审查发现，`SC-M05-QA-001 P2-d`） | `系统-经营与成长` §6.1 规定未冻结时显示 **`— / —`、不显示分子**；而 `Boot.ts` 与 `MallScene.ts` 实际显示 **`0/—`**（分子取此刻恒为 0 的 `visitorsServed`） | **仍成立（非 M05 引入）**。`0/—` 是 M04 已记录并验收通过的实测读数（`docs/M04-客户端工程与主界面.md` §1/§4），M05 沿用未改以免连带推翻它。语义差别：`0/—` 会被读成"今天还没客人、上限未知"，`— / —` 才是"上限尚不适用"。**需主理人裁定改哪一侧**——改实现须同步回改 M04 的读数 |
