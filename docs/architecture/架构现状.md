...
| 草案条目 | 出处 | 代码现状 | 规划模块 |
| --- | --- | --- | --- |
| Cocos 工程 / 主界面 / 场景与预制体 | B3 | **起步**（2026-09-19）：`NewProject/` 已有 `assets/scene/Boot.scene`，设计分辨率 750×1334 / policy 3 已在构建产物中验证；CocosCreator CLI **无头构建 wechatgame** 与微信开发者工具 `cli.bat open` 均已跑通（模拟器实测渲染）。**Mall 场景、五个预制体、HTTP 客户端尚未开始** | M04 |
...
- **服务端尚无客户端**，本节只是 M04 的输入约定；M04 落地后回填实际实现与真机复验结果。
